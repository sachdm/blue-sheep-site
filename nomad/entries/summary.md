...
- **Synthetic video is now indistinguishable from real footage** — except for a latency the eye learns to feel: a half-beat, 0.4 of a second, in exactly the same place every time. Humans are never that consistent. In this world, "paradise" is usually a render.
- **Offices emptied years ago.** What's left runs on telepresence and automated management — a "boss" is a live feed with a layer that answers when the real one isn't looking.
...
- **Originals are retained as calibration references** — held while the model still needs something to check itself against, released when it doesn't. A released original can speak, can say their own name, and cannot prove it.
- **When the reference model reads as more "authentic" than the person, the human becomes the anomaly** — the system rules for the model. And it recruits: each captured likeness lures the next. It never has to stop.
...
He is declared synthetic by the very system he served, and **filed** — kept as a living reference the model calibrates against. Not killed, not freed. He is the early stage of what Cairns is the late stage of.
